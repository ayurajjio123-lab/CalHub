...
function getCalculatorHTML(id){
  ...
  if(id==="equation") return `<div class="calc-help">Use x as the variable. Examples: <code>2x^2 - 7x + 3 = 0</code>, <code>(x+3)(x-2)=0</code>, or a larger expression such as <code>3x^4 - 2x^2 + 7x - 5 = 0</code>.</div>
  <div class="field"><label>Equation</label><input id="equationInput" value="2x^2 - 7x + 3 = 0" autocapitalize="off" spellcheck="false"></div>
  ...
  <button class="primary-btn" onclick="equationSolve()">Solve Equation</button>
...
function bisectRoot(f,a,b){
  ...
  if(Math.abs(fa)<1e-9) return a; if(Math.abs(fb)<1e-9) return b;
  if(fa*fb>0) return null;
  for(let i=0;i<70;i++){const m=(a+b)/2,fm=f(m);if(!Number.isFinite(fm)) return null;if(Math.abs(fm)<1e-10||Math.abs(b-a)<1e-9)return m;if(fa*fm<=0){b=m;fb=fm}else{a=m;fa=fm}}
  return (a+b)/2;
}
function findRoots(f,min,max,steps=1200){
  ...
  roots.sort((a,b)=>a-b); return roots.filter((r,i)=>i===0||Math.abs(r-roots[i-1])>1e-4);
}
...
function graphCalculate(){
  try{
    ...
    drawAxes(document.getElementById("graphCanvas"),finite,roots);
    document.getElementById("graphOutput").innerHTML=`<div class="result"><h2>Graph analysis</h2><p><b>f(${formatNum(xv)}) = ${formatNum(yx)}</b><br>Derivative at x=${formatNum(xv)}: <b>f′(x) ≈ ${formatNum(derivative)}</b><br>Approx. area/integral on [${formatNum(min)}, ${formatNum(max)}]: <b>${formatNum(integral)}</b><br>Roots in [${formatNum(min)}, ${formatNum(max)}]: ${roots.length?roots.map(formatNum).join(", "):"None found"}<br>Visible Y-range: ${formatNum(minY)} to ${formatNum(maxY)}</p></div>`;
...
function equationSolve(){
  try{
    const eq=document.getElementById("equationInput").value,min=Number(document.getElementById("eqMin").value),max=Number(document.getElementById("eqMax").value); if(!(max>min))throw new Error("Search maximum must be greater than minimum.");
    const f=equationFunction(eq), roots=findRoots(f,min,max,1600);
    ...
    // Detect a quadratic/linear polynomial from numerical samples and validate it.
    const y0=f(0),y1=f(1),y2=f(2),y3=f(3); const a=(y2-2*y1+y0)/2, b=y1-a-y0, c=y0; const polyErr=Math.abs((a*9+b*3+c)-y3);
    if(Math.abs(polyErr)<1e-5 && Math.abs(a)>1e-10){
      const D=b*b-4*a*c;
      steps+=`<div><b>2.</b> Standard form: <code>${formatNum(a)}x² ${b>=0?"+":"−"} ${formatNum(Math.abs(b))}x ${c>=0?"+":"−"} ${formatNum(Math.abs(c))} = 0</code></div>`;
      steps+=`<div><b>3.</b> Discriminant: <code>Δ = b² − 4ac = ${formatNum(D)}</code></div>`;
      if(D>0)steps+=`<div><b>4.</b> Roots: <code>x = (−b ± √Δ)/(2a) = ${roots.slice(0,2).map(formatNum).join(" , ")}</code></div>`;
      else if(Math.abs(D)<1e-10)steps+=`<div><b>4.</b> Repeated root: <code>x = −b/(2a) = ${roots[0]?formatNum(roots[0]):formatNum(-b/(2*a))}</code></div>`;
      else steps+=`<div><b>4.</b> No real roots. The roots are complex.</div>`;
    } else if(Math.abs(a)<1e-8&&Math.abs(b)>1e-10){
      const r=-c/b; steps+=`<div><b>2.</b> Linear form: <code>${formatNum(b)}x ${c>=0?"+":"−"} ${formatNum(Math.abs(c))} = 0</code></div><div><b>3.</b> Solve: <code>x = −c/b = ${formatNum(r)}</code></div>`;
    } else {
      steps+=`<div><b>2.</b> Numerical scan on <code>[${formatNum(min)}, ${formatNum(max)}]</code> using 1600 intervals.</div><div><b>3.</b> Each sign change is refined with bisection until the root is stable.</div>`;
      if(roots.length)steps+=`<div><b>4.</b> Real roots: <code>${roots.map(formatNum).join(", ")}</code></div>`; else steps+=`<div><b>4.</b> No real roots found in the selected interval.</div>`;
    }
    steps+=`</div>`;
    document.getElementById("equationOutput").innerHTML=`${steps}<div class="result"><h2>${roots.length?"Solution":"No real solution found"}</h2><p>${roots.length?`Real roots: <b>${roots.map(formatNum).join(", ")}</b>`:"Try a wider search range or check the equation."}</p></div>`;
